...
products.forEach((product, index) => {
  const whatsappText = `I am Intereseted in ${product.name}:  ${origin + pathname
    }#product-${index}`;
  const productCard = `
        <div class="flex flex-col justify-between bg-white shadow rounded-2xl hover:shadow-2xl p-4" id="product-${index}">
          ...
          <h3 class="text-xl font-semibold mb-1 text-gray-900">${product.name
    }</h3>
          <p class="text-gray-600 mb-2">${product.description}</p>
          <p class="font-bold text-amber-700 mb-3">₹ ${product.price}</p>
          <a href="https://wa.me/917738789682?text=${encodeURIComponent(
      whatsappText
    )}"
            target="_blank"
            class="block text-center px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 transition duration-300">
            Buy on WhatsApp
          </a>
        </div>
      `;
